...
def make_quality_compression(original,sal):
    ...
    img_qualities = []
    quality_steps = [i*i for i in range(1,11)]
    #quality_steps = [1, 2, ]
    #quality_steps = [i for i in range(1,11)] #modified
    print('quality steps')
    print(quality_steps)
    ...
    shape = k.shape 
    #k.flags.writeable = True prev version?
    k.setflags(write=1)
    ...
    print("percentile split ups")
    print(q_a)
    low, med, high = 2, 5, 9


    data_final = []
    data_final = np.zeros((shape[0], shape[1], shape[2]))
    filter_size = 4
    temp = []
    for i in range(shape[0]):
        for j in range(shape[1]):
            for l in range(filter_size):
                if i + l - filter_size < 0 or i + l - filter_size > shape[0] - 1:
                    for c in range(filter_size):
                        temp.append(0)
                else:
                    if j + l - filter_size < 0 or j + filter_size > shape[1] - 1:
                        temp.append(0)
                    else:
                        for m in range(filter_size):
                            temp.append(sal_arr[i + l - filter_size, j + m - filter_size, 0])
                

            temp.sort()
            data_final[i, j, 0] = float(temp[len(temp) // 2])
            temp = []
    
    print(data_final)
    img = Image.fromarray( data_final[:,:,0].reshape((shape[0], shape[1])))
    #img.show()
    #out_img = array2PIL(data_final)
    #rgb_out = out_img.convert('RGB')
    #out_name = args.output_directory + '/' + '_filter_' + args.image.split('/')[-1] + '_' + '.png'
    ...
    if args.find_best:
        ...
        for qual in range(4,2,-1):
            rgb_out.save(out_name, quality=50)
...
if False:
    ...
    sal_rgb = sal.convert('RGB')
    a,b = make_quality_compression(original_rgb, sal_rgb)
    ...
    for image_file in glob(image_path):
        ...
        map_file = 'depth-map/' + image_file.split('/')[1].split('.')[0] + '.png'
